...
if (count($data)>0){
if (password_verify( $TBpassword, $data[0]['Password'])) {
  ...
  if (isset($data[0]['Username']) &&isset($data[0]['Password'])) {
    ...
    if ($base64_hash === $signature) {
        ...
        // set cookie
        setcookie('token', $jwt_token, time() + (86400 * 1), '/', "", true, true);
        
        // Return token and user data as JSON response
    if ($TBusername == 'admin'&&$TBpassword == 'admin'){
      $response = [
        'username' => 'admin'
      ];
